...
library(mlogit)
d<-read.csv("CE_R_p75.csv")
data<-mlogit.data(d,choice="choice",shape="wide",varying=7:474,
                  id="id",sep="")

library(reshape)
data<-rename(data,c(pf_pregnant = "p75_pregnant", pf_child = "p75_child", 
                    pf_baby = "p75_baby", pf_protection = "p75_protection", 
                    pf_ownrisk = "p75_ownrisk", pf_duration = "p75_duration", 
                    info_pf_pregnant="info_p75_pregnant", 
                    info_pf_child = "info_p75_child", 
                    info_pf_baby = "info_p75_baby", 
                    info_pf_protection = "info_p75_protection", 
                    info_pf_ownrisk = "info_p75_ownrisk", 
                    info_pf_duration="info_p75_duration"))

set.seed(98457)
save.image("session-main_p75.RData")
draws<-1000

baseline.mxl<-mlogit(choice~price+asc+pregnant+baby + child + protection + 
                     own_risk + duration + info_pregnant + info_child + info_baby + 
                     info_protection + info_ownrisk + info_duration + p75_pregnant + 
                     p75_child + p75_baby + p75_protection + p75_ownrisk + 
                     p75_duration + info_p75_pregnant + info_p75_child + 
                     info_p75_baby + info_p75_protection + info_p75_ownrisk + 
                     info_p75_duration|0,data, rpar=c(asc='n', pregnant ='n', child = 'n', 
                     baby='n', protection='n',own_risk='n', duration='n', info_pregnant='n', 
                     info_child='n', info_baby='n', info_protection='n', info_ownrisk='n', 
                     info_duration='n', p75_pregnant='n', p75_child='n', p75_baby='n', 
                     p75_protection='n', p75_ownrisk='n', p75_duration='n', 
                     info_p75_pregnant='n', info_p75_child='n', info_p75_baby='n',
                     info_p75_protection='n', info_p75_ownrisk='n', info_p75_duration='n'),
...
noschool.mxl<-mlogit(choice~price+asc+pregnant+baby + child + protection + 
                     ...
                     p75_pregnant + p75_child + p75_baby + p75_protection + p75_ownrisk +
                     p75_duration + info_p75_pregnant + info_p75_child + 
                     info_p75_baby + info_p75_protection + info_p75_ownrisk + 
                     info_p75_duration|0,data, rpar=c(asc='n', pregnant ='n', child = 'n', 
                     baby='n', protection='n',own_risk='n', duration='n', info_pregnant='n', 
