...
library(mlogit)
d<-read.csv("CE_R_p50.csv")
data<-mlogit.data(d,choice="choice",shape="wide",varying=7:474,
                  id="id",sep="")

library(reshape)
data<-rename(data,c(pe_pregnant = "p50_pregnant", pe_child = "p50_child", 
                    pe_baby = "p50_baby", pe_protection = "p50_protection", 
                    pe_ownrisk = "p50_ownrisk", pe_duration = "p50_duration", 
                    info_pe_pregnant="info_p50_pregnant", 
                    info_pe_child = "info_p50_child", 
                    info_pe_baby = "info_p50_baby", 
                    info_pe_protection = "info_p50_protection", 
                    info_pe_ownrisk = "info_p50_ownrisk", 
                    info_pe_duration="info_p50_duration"))

set.seed(98457)
save.image("session-main_p50.RData")
draws<-1000

baseline.mxl<-mlogit(choice~price+asc+pregnant+baby + child + protection + 
                     own_risk + duration + info_pregnant + info_child + info_baby + 
                     info_protection + info_ownrisk + info_duration + p50_pregnant + 
                     p50_child + p50_baby + p50_protection + p50_ownrisk + 
                     p50_duration + info_p50_pregnant + info_p50_child + 
                     info_p50_baby + info_p50_protection + info_p50_ownrisk + 
                     info_p50_duration|0,data, rpar=c(asc='n', pregnant ='n', child = 'n', 
                     baby='n', protection='n',own_risk='n', duration='n', info_pregnant='n', 
                     info_child='n', info_baby='n', info_protection='n', info_ownrisk='n', 
                     info_duration='n', p50_pregnant='n', p50_child='n', p50_baby='n', 
                     p50_protection='n', p50_ownrisk='n', p50_duration='n', 
                     info_p50_pregnant='n', info_p50_child='n', info_p50_baby='n',
                     info_p50_protection='n', info_p50_ownrisk='n', info_p50_duration='n'),
...
noschool.mxl<-mlogit(choice~price+asc+pregnant+baby + child + protection + 
                     ...
                     p50_pregnant + p50_child + p50_baby + p50_protection + p50_ownrisk +
                     p50_duration + info_p50_pregnant + info_p50_child + 
                     info_p50_baby + info_p50_protection + info_p50_ownrisk + 
                     info_p50_duration|0,data, rpar=c(asc='n', pregnant ='n', child = 'n', 
                     baby='n', protection='n',own_risk='n', duration='n', info_pregnant='n', 
